                ...
                <div class="space-y-4">
                    ...
                    @else
                        @if($retainedPlayers->isNotEmpty())
                            <div class="rounded-2xl border border-amber-200/20 bg-amber-200/5 p-5">
                                ...
                                <div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
                                    @foreach($retainedPlayers as $player)
                                        ...
                                        @endphp
                                        <div class="group rounded-2xl border border-white/15 bg-white/5 p-4 text-white transition hover:-translate-y-1 hover:border-amber-300/70">
                                            <div class="relative mb-3">
                                                <img src="{{ $photoUrl }}" alt="{{ $player->user->name }}" class="w-full rounded-2xl object-cover aspect-[4/5]" loading="lazy" crossorigin="anonymous">
                                                <span class="absolute top-3 left-3 flex h-7 w-7 items-center justify-center rounded-full bg-amber-300/90 text-slate-900 shadow-lg shadow-amber-500/40">
                                                    <svg class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 20 20">
                                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                    </svg>
                                                </span>
                                            </div>
                                            <p class="text-sm font-semibold leading-tight text-center sm:text-base">{{ $player->user->name }}</p>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
