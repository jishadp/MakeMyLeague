                                                ...
                                                <div class="text-center">
                                                    <div class="relative inline-block">
                                                        @if($player->user->photo)
                                                            <img src="{{ Storage::url($player->user->photo) }}" class="w-10 h-10 rounded-full object-cover mb-1 {{ ($player->retention == 1 || $player->retention === true) ? 'border-yellow-400 border-2 ring-2 ring-yellow-200' : 'border border-gray-200' }}">
                                                        @else
                                                            <div class="w-10 h-10 rounded-full flex items-center justify-center mb-1 {{ ($player->retention == 1 || $player->retention === true) ? 'bg-yellow-100 border-2 border-yellow-400 ring-2 ring-yellow-200' : 'bg-gray-100 border border-gray-200' }}">
                                                                <span class="text-xs font-bold {{ ($player->retention == 1 || $player->retention === true) ? 'text-yellow-600' : 'text-gray-600' }}">{{ substr($player->user->name, 0, 1) }}</span>
                                                            </div>
                                                        @endif
                                                        @if($player->retention == 1 || $player->retention === true)
                                                    ...
                                                    <p class="text-xs {{ ($player->retention == 1 || $player->retention === true) ? 'text-yellow-700 font-semibold' : 'text-gray-900' }} truncate">{{ explode(' ', $player->user->name)[0] }}</p>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
