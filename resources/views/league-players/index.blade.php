    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @auth
            @if(auth()->user()->isAdmin() || auth()->user()->isOrganizerForLeague($league->id))
                ...
                <!-- Clear All Retention Card -->
                @php
                    $retainedPlayersCount = $league->leaguePlayers()->where('retention', true)->count();
                @endphp
                <div class="bg-white rounded-lg shadow-sm p-6 mb-8">
                    <div class="flex items-start justify-between gap-4 flex-col md:flex-row">
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900">Clear All Retention</h2>
                            <p class="text-sm text-gray-600 mt-1">
                                Remove retention status and team assignment from <span class="font-semibold text-red-700">{{ $retainedPlayersCount }}</span> currently retained player(s). This will move them back to the auction pool.
                            </p>
                        </div>
                        <form method="POST" action="{{ route('league-players.bulk-clear-retention', $league) }}" class="flex items-end">
                            @csrf
                            <button type="submit"
                                    @if($retainedPlayersCount === 0) disabled @endif
                                    onclick="return confirm('Are you sure you want to clear retention status for ALL retained players? This will also remove them from their teams.');"
                                    class="inline-flex items-center px-4 py-2 text-white font-medium rounded-lg transition-colors text-sm {{ $retainedPlayersCount === 0 ? 'bg-gray-300 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700' }}">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
