...
@section('content')
    @php
        ...
        $currentBids = $currentBids ?? collect();
        $teams = $teams ?? collect();
        $liveViewers = $liveViewers ?? 0;
        $lastOutcomePlayer = $lastOutcomePlayer ?? null;
        $recentSoldPlayers = $recentSoldPlayers ?? collect();
        $bidStartTime = $currentPlayer?->updated_at ?? null;
        $lastUpdated = $lastUpdated ?? now()->format('H:i:s');
    ...
    <input type="hidden" id="league-id" value="{{ $leagueModel->id }}">

    <div id="broadcastRoot" data-broadcast-theme="dark">
                ...
                <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    ...
                    <div class="flex flex-wrap gap-4 sm:gap-6 text-sm items-start sm:items-center justify-start lg:justify-end">
                        ...
                        <div>
                            ...
                            <p class="text-3xl font-semibold leading-tight">{{ $lastUpdated }}</p>
                        </div>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const attachChannelHandlers = (pusherChannel) => {
                ...
                ['player-sold', 'player-unsold', 'new-player-started', 'new-player-bid-call'].forEach(eventName => {
                    pusherChannel.bind(eventName, scheduleReload);
                });
            };

            attachChannelHandlers(window.leagueChannel);
            attachChannelHandlers(window.channel);
