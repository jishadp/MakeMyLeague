...
@section('title', 'Edit League - Cricket League Manager')
...
@section('content')
    <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-lg p-8">
            <h2 class="text-3xl font-bold text-gray-900 mb-8 tracking-tight">Edit League</h2>
            
            <form action="{{ route('leagues.update', $league) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-2">League Name</label>
                    <input type="text" name="name" id="name" required value="{{ old('name', $league->name) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" required value="{{ old('start_date', $league->start_date->format('Y-m-d')) }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-lg">
                        @error('start_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" id="end_date" required value="{{ old('end_date', $league->end_date->format('Y-m-d')) }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-lg">
                        @error('end_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
