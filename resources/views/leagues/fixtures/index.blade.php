...
<div class="bg-slate-50 text-slate-900">
                                    ...
                                    @if(auth()->check())
                                        <div class="px-4 py-2 bg-slate-50 border-t border-slate-100 flex justify-end gap-2">
                                            @if(auth()->user()->isOrganizerForLeague($league->id) || auth()->user()->isAdmin())
                                                ...
                                                <button onclick="confirmDeleteMatch('{{ $fixture->slug }}', '{{ $fixture->homeTeam->team->name }}', '{{ $fixture->awayTeam->team->name }}')" class="text-xs bg-red-100 text-red-700 px-2 py-1 rounded hover:bg-red-200">
                                                    Delete Match
                                                </button>
                                            @endif
    ...
    <div id="delete-match-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" onclick="closeDeleteMatchModal()"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Delete Match</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Are you sure you want to delete the match <strong id="delete-match-teams"></strong>? This action cannot be undone. All match data including scores, events, and penalties will be permanently deleted.
                                </p>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    let deleteMatchSlug = null;

    function confirmDeleteMatch(fixtureSlug, homeTeam, awayTeam) {
        deleteMatchSlug = fixtureSlug;
        document.getElementById('delete-match-teams').textContent = `${homeTeam} vs ${awayTeam}`;
        document.getElementById('delete-match-modal').classList.remove('hidden');
    }

    function closeDeleteMatchModal() {
        deleteMatchSlug = null;
        document.getElementById('delete-match-modal').classList.add('hidden');
    }

    function executeDeleteMatch() {
        if (!deleteMatchSlug) return;

        const btn = document.getElementById('confirm-delete-btn');
        btn.disabled = true;
        btn.textContent = 'Deleting...';

        fetch(`/leagues/{{ $league->slug }}/fixtures/${deleteMatchSlug}`, {
            method: 'DELETE',
            headers: {
