...
@section('title', 'Admin - Edit Local Body - ' . config('app.name'))
...
<section class="py-12 px-4 sm:px-6 lg:px-8 bg-gray-50">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8 animate-fadeInUp">
            <form action="{{ route('admin.locations.update-local-body', $localBody) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <select id="district_id" 
                        ...
                        @foreach($districts as $district)
                            <option value="{{ $district->id }}" 
                                    {{ old('district_id', $localBody->district_id) == $district->id ? 'selected' : '' }}>
